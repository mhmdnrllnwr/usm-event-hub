...
class TestFormatEventRendering:
    ...
    def test_partial_time_no_end(self):
        """When end_time missing, show start_time alone (not 'empty')."""
...
class TestNlpEntityFallback:
    ...
    def test_date_with_calendar_vs16_and_english_month(self):
        """🗓️ (U+1F5D3 + U+FE0F) + English month. Regression test for
        noisy emoji-heavy text where spaCy misses the date."""
        text = "🎙️ Quote line\n\n🗓️ 21 May 2026\n⏰ 8:00 PM – 10:00 PM\n💻 Cisco Webex\n💰 Fee: Only RM1"
        result = extract_entities(text)
        assert "21 May 2026" in result.get("date_spacy", [])
